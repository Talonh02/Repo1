...
-   Collie is a more efficient runner and is less adversely affected by inclines while also showing efficiency gains over longer distances
...
likelihood <- tidied$p.value
likelihood_pct <- likelihood * 100
likelihood_formatted <- format(likelihood_pct, scientific = FALSE)
...
CollieTE <- aeroTEcollie %>% ggplot(mapping = aes(x = AeroTEnum,  y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(bins = 10, col = "black",fill = "red") +
    labs(title = "Collie Aerobic TE distribution", y = "Aerobic TE Percentages", x = "Aerobic TE") +
...
HoundTE <- aeroTEhound %>% ggplot(mapping = aes(x = AeroTEnum, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  ...
  labs(title = "Hound Aerobic TE distribution", y = "Aerobic TE Percentages", x = "Aerobic TE") +
  scale_y_continuous(labels = scales::percent_format(scale = 1))
...
Starting in 2020, Collie's running efficiency has had a consistency that was comparable to Hound's. Over time, Collie's performance shows a steady improvement with similar variability but a clear upward trend in efficiency. Despite a temporary break from running that takes place between late 2021 and continuing into 2022, Collie's running efficiency continues with an upward trend and consistent improvement. These findings reveal that, while Hound’s top end efficiency had improved, he also displayed a lack of consistency. In contrast, Collie demonstrates much greater consistency and steady improvement over time.
...
There are a myriad of factors that could affect a runner's ability to run at their typical pace or efficiency. It is important to study how adverse conditions affect each runner, as if one runner has an Achilles heel when it comes to a factor such as incline, heat, or very long distances, they would not be as suitable for high-intensity races that involve varied terrain, hot weather, or long endurance competitions. Out of those three factors, it appears that slope has a very strong effect on efficiency for both runners while the other two factors play less of a role.
